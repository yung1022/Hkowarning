...
def estimate_pixel_rain_metrics(pixel):
    ...
    # 3. Detect if the precipitation is rain or snow
    # The highest bit (128 flag) isolates frozen precipitation from liquid
    if (r & 128) != 0:
        dbz = float('-inf')  # Snow or frozen precipitation, ignore for rain metrics

    # 4. Turn clear/no-echo areas (fully transparent background pixels) into clean boundary markers
    dbz = float('-inf') if a == 0 else dbz
